...
export default function ImportantQuestions() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/api/important-questions`);
        const formattedData = res.data
          .filter((item) => item.course?.toLowerCase() === "board" && item.url)
          .map((item) => ({
            ...item,
            ...
            title: `Board Important Questions (${item.year})`
          }));
  ...
  const handleDownload = async (item) => {
    try {
      ...
      a.href = url;
      a.download = `Board-Important-Questions-${item.year}-${item.filename || 'questions'}.pdf`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-16 px-6 md:px-12 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading important questions...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Access and download Board exam important questions for classes 10th and 12th.
            All questions are available in PDF format to help you prepare effectively.
          </p>
        </div>
        ...
        {questions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">No Board important questions available yet.</p>
            <p className="text-gray-400">Check back later for updated questions.</p>
          </div>
        ) : (
