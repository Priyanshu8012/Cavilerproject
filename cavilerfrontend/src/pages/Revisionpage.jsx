...
export default function RevisionNotes() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const res = await axios.get(`${BASE_URL}/api/revision-notes`);
        const formattedData = res.data
          ...
          .map((item) => ({
            ...
            title: `Revision Notes (${item.year})`
          }));
  ...
  const handleDownload = async (item) => {
    try {
      ...
      a.href = url;
      a.download = `Foundation-Revision-Notes-${item.year}-${item.filename || 'notes'}.pdf`;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-16 px-6 md:px-12 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading revision notes...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Access and download comprehensive revision notes for Foundation courses.
            All notes are available in PDF format to help you revise effectively.
          </p>
        </div>
        ...
        {notes.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">No revision notes available yet.</p>
            <p className="text-gray-400">Check back later for updated notes.</p>
          </div>
        ) : (
