...
import { FaCloudUploadAlt, FaFilePdf, FaTrashAlt, FaHistory, FaFilter, FaBookOpen } from "react-icons/fa";

const SamplePaperUpload = () => {
  ...
  // Use the VITE_BASE_URL from the environment variables
  const API_URL = import.meta.env.VITE_BASE_URL + "/api/sample-papers";

  // Fetch all sample papers on mount
  useEffect(() => {
    fetch(API_URL)
      .then((res) => res.json())
      .then((data) => setSamplePapers(data))
      .catch(console.error);
  }, []);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      setSamplePapers((prev) => [data, ...prev]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setSamplePapers((prev) => prev.filter((item) => item.id !== id));
    } catch (error) {
      console.error(error);
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <span className="px-3 py-1 bg-cavalier-header-bg/30 rounded-full text-xs font-bold text-gray-400 border border-white/5">
                  {samplePapers.length} Total Resources
                </span>
              </div>
