...
export default function AdminPriceUpdate() {
    ...
    // Fetch latest price details from backend
    useEffect(() => {
        const fetchPriceDetails = async () => {
            try {
                const response = await fetch(`${API_BASE_URL}/api/neet-update-price`);
    ...
    // Calculate discounted price and EMI
...
<div className="bg-gray-900 shadow-lg rounded-2xl p-8 border border-gray-700 w-full max-w-lg">
                    ...
                    {[
                        ...
                        <div key={index}>
                            ...
                            <input
                                ...
                                className="w-full p-3 border border-gray-600 rounded-md focus:ring-2 focus:ring-orange-500 focus:outline-none bg-gray-800 text-white"
                            />
                        </div>
                    ))}
                ...
                {finalPrice && (
                    <div className="mt-6 p-4 bg-gray-800 border border-orange-500 rounded-md text-center">
                        <p className="text-gray-400 line-through">₹{originalPrice}</p>
                        <p className="text-orange-400 font-bold">{discount}% OFF</p>
                        <h3 className="text-2xl font-extrabold text-white">₹{finalPrice} for {duration} years</h3>
