...
import { FaCloudUploadAlt, FaFilePdf, FaTrashAlt, FaHistory, FaFilter, FaStickyNote } from "react-icons/fa";
...
const NCRTsolutions = () => {
  ...
  // Fetch all uploaded solutions from the server
  const fetchUploadedSolutions = async () => {
    try {
      const response = await axios.get(`${API_BASE}/api/ncrt-solutions`);
      setUploadedSolutions(response.data);
    } catch (error) {
      console.error("Error fetching uploaded solutions:", error);
    }
  };

  // Fetch the uploaded solutions initially when the component mounts
  useEffect(() => {
    fetchUploadedSolutions();
  }, []);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE}/api/ncrt-solutions/upload`,
        formData,
        {
      ...
      // Update uploaded solutions list
      setUploadedSolutions((prev) => [newSolution, ...prev]);
      setUploadedData(newSolution);
      setSelectedCourse("");
      setYear("");
      setFile(null);
  ...
  const handleDelete = (index) => {
    ...
    axios
      .delete(`${API_BASE}/api/ncrt-solutions/${uploadedSolutions[index].id}`)
      .then((response) => {
          ...
          <div className="md:w-1/2">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-cavalier-brand mb-8 flex items-center gap-3 uppercase tracking-tighter">
                <FaStickyNote className="text-4xl" />
                Upload Solutions
              </h2>
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <span className="px-3 py-1 bg-cavalier-header-bg/30 rounded-full text-xs font-bold text-gray-400 border border-white/5">
                  {uploadedSolutions.length} Total Resources
                </span>
              </div>
              ...
              <div className="space-y-4 flex-grow overflow-y-auto pr-2 custom-scrollbar" style={{ maxHeight: 'calc(100vh - 350px)' }}>
                {uploadedSolutions.length === 0 ? (
                  ...
                  uploadedSolutions.map((solution, index) => (
                    <motion.div
                      layout
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-black text-cavalier-brand uppercase tracking-tighter">{formatCourse(solution.course)}</span>
                            <span className="w-1 h-1 bg-gray-600 rounded-full"></span>
                            <span className="text-xs font-bold text-gray-400">{solution.year}</span>
                          </div>
                          <p className="text-sm font-bold text-white mt-1 group-hover:text-cavalier-brand transition-colors line-clamp-1">{solution.filename}</p>
                        </div>
                      </div>
